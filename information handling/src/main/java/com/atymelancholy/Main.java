...
import com.atymelancholy.reader.FileTextReader;
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        FileTextReader textFileService = new FileTextReader();
        TextParserService parserService = new TextParserService();
        TextRestorer restorer = new TextRestorer();
        TextStatisticsService statisticsService =
                new TextStatisticsService();
        TaskOperationsService operationsService =
                new TaskOperationsService();
        ...
        try {
            ...
            String rawText =
                    textFileService.read(inputFilePath);
            ...
            LOGGER.info(
                    "Letter count: {}",
                    statisticsService.countLetters(parsedText)
            );

            LOGGER.info(
                    "Symbol count: {}",
                    statisticsService.countSymbols(parsedText)
            );
            ...
            LOGGER.info(
                    "Max sentence overlap count: {}, words: {}",
                    overlapResult.sentenceCount(),
                    overlapResult.words()
            );

            LOGGER.info(
                    "Sentences sorted by letter 'a': {}",
                    operationsService.sortSentencesByLetterCount(parsedText, 'a')
            );

            LOGGER.info(
                    "Text after swapping first/last lexeme:\n{}",
    ...
    private static String resolveInputFilePath(String[] args) {
        ...
        if (args.length == 0 || args[0].isBlank()) {
            ...
            LOGGER.warn(
                    "No input file argument provided. Falling back to default file: {}",
                    DEFAULT_INPUT_FILE
            );
